      ...
      for (ip3 in 1:max(1,numtype[3])) {
        for (ip4 in 1:max(1,numtype[4])) {
          for (ip5 in 1:max(1,numtype[5])) {
            ...
            newname <- paste(substr(xmlList[ix],1,2), substr(paramList1[[1]][ip],1,2), substr(xmlList[ix],9,16), sep = "_") # Change the output file name to insert the harvest type only
